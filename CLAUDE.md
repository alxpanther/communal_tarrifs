...
| Per-city sources | `src/common/ai_pipeline.py` | RU, AM, AZ, MD, GE, TJ | Each city declares its own sources in config and is read separately |
| Config-declared | `src/common/manual_pipeline.py` | BY, KG, KZ, TM, UZ | **Unfinished.** Numbers sit in config instead of being collected — see the migration note below |
