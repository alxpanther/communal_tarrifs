...
9. **Жёсткая валидация тарифов на воду:** Ответ модели сверяется с исходной таблицей — каждая тройка «водопостачання / водовідведення / разом» должна встречаться в источнике именно в таком порядке, количество строк и название поставщика обязаны совпадать. Если хоть что-то не сошлось, старые данные остаются нетронутыми, а в Telegram уходит перечень претензий.
10. **Стабильные `city_code` (`config/city_registry.json`):** Идентификаторы городов не зависят от ответа модели — они хранятся в постоянном реестре и после первого назначения никогда не меняются. Android-приложение хранит `city_code` как выбор пользователя, поэтому его «дрейф» между запусками недопустим.
