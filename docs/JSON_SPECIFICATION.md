...
| Поле | Тип | Описание | Пример |
|---|---|---|---|
| `city_code` | `String` | Уникальный латинский идентификатор города (slug, Primary Key для приложения). | `"kyiv"`, `"lviv"` |
| `city_name` | `String` | Название города/региона на украинском языке для отображения в UI. | `"Київ"`, `"Львів"` |
| `supplier` | `String` | Наименование предприятия-поставщика (водоканала). | `"ПрАТ АК 'Київводоканал'"` |
| `water_supply` | `Double` | Тариф за 1 м³ централизованного водоснабжения (холодная вода) в UAH. | `22.884` |
| `sewage` | `Double` | Тариф за 1 м³ водоотведения (канализация) в UAH. | `16.548` |
| `total_rate` | `Double` | Суммарный тариф (водоснабжение + водоотведение) за 1 м³ в UAH. | `39.432` |
...
| `decree_info` | `String` | Номер и реквизиты постановления регулятора (НКРЕКП). | `"Постанова НКРЕКП № 2842"` |
