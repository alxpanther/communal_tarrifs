...
| `tier_basis` | `String?` | Как применяются ступени потребления: `"part"` — каждая часть потребления оплачивается по цене своей ступени; `"whole"` — всё потребление по цене ступени, до которой оно дошло; `null` — ступеней нет или источник не говорит. | `"part"` |
...
Цена плана выбирается так же, как цена электроэнергии (раздел 4.1, шаги 2–5): строки, сезон которых покрывает месяц; при ступенях — ступень, до которой дошёл расход, считая за месяц или за год по `tier_period` и деля по `tier_basis`, как `tier_basis` электроэнергии (шаг 4); плюс `monthly_charge`, если он задан. Ступени Азербайджана годовые и `"part"`: первые 1200 м³ года по первой цене, следующие до 2500 — по второй, остальное — по третьей.
