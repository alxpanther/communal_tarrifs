...
| `certs/*.pem` | Владелец | Промежуточные сертификаты, которые сайт-источник не отдаёт сам; каждый скачан по адресу издателя из сертификата этого сайта. Дополняют стандартные корни, но не заменяют их. |
